...
ACTION_NOISE = 0.20      # Standard deviation reference for action noise
PARAM_NOISE = 0.20      # reference for parameter noise
...
class DDPG():
    ...
    def __init__(self, env, state_size, action_size, seed, device, fc_units_act=(64, 64), fc_units_critic=(64, 64), parameter_noise=True, correlated_action_noise = False):
        ...
        self.DDPGActor.model[-1].weight.data.uniform_(-3e-3, 3e-3)
    ...
    def step(self, max_t=1001): 
        ...
        episode_dones = False
        if self.t_step >= max_t or np.any(dones):
            episode_dones = True
        
        # Update the model every period of step and when Replay memory is enough large
        if len(self.memory) > BATCH_SIZE and self.total_t_step % T_UPDATE == 0:
    ...
    def learn(self, n_episodes=200, max_t=2000, save=False, target=10000.):
        ...
        for i_episode in range(1, n_episodes+1):
            ...
            if np.mean(scores_window)>=target:
                ...
                scores_avg.append(np.mean(scores_window))
                if save:
                    torch.save(self.DDPGActor.state_dict(), 'DDPGActor_Crawler_model.pt')
    ...
    def play(self):
        ...
        if os.path.isfile('DDPGActor_Crawler_model.pt'):
            print('saved model file has been found and loaded')
            self.load('DDPGActor_Crawler_model.pt')
        else:
...
class RunningNormalizer:
    ...
    def save(self, name='DDPGNormalizer_Crawler'):
    ...
    def load(self, name='DDPGNormalizer_Crawler'):
